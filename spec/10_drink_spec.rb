...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:my_cola) { described_class.new('cola') }
      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(my_cola.type).to eq('cola')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(my_cola.ounces).to eq(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:my_tea) { described_class.new('tea',16) }
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:my_coffee) { described_class.new('coffee', 12)}

      # remove the 'x' before running this test
      it 'is not full' do
        expect(my_coffee).to_not be_full
      end
    end
  end
end
