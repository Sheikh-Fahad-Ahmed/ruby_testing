...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      subject(:game_not_over) { described_class.new(7, '3')}
      # remove the 'x' before running this test
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        ...
        alphabet = 'c'
        valid_input = '5'
        allow(game_loop).to receive(:player_input).and_return(two_digits, alphabet, valid_input)
      end
  ...
  describe '#final_message' do
    ...
    context 'when count is 4 and over' do
      subject(:game_three) { described_class.new(6, '6', 7)}
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        hard_win_phrase = "That was hard. It took you 7 guesses!\n"
        expect { game_three.final_message }.to output(hard_win_phrase).to_stdout
      end
    end
  end
end
